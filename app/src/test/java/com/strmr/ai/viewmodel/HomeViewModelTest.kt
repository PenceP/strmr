...
class HomeViewModelTest {
    ...
    @Test
    fun `initial state has loading continue watching`() = runTest {
        // Given
    ...
    @Test
    fun `checkTraktAuthorization updates state to authorized when use case returns true`() = runTest {
        // Given
        whenever(getContinueWatchingUseCase.refreshContinueWatching()).thenReturn(Result.success(emptyList()))
    ...
    @Test
    fun `checkTraktAuthorization updates state to not authorized when use case returns false`() = runTest {
        // Given
        whenever(getContinueWatchingUseCase.refreshContinueWatching()).thenReturn(Result.success(emptyList()))
        whenever(checkTraktAuthorizationUseCase()).thenReturn(Result.success(false))
    ...
    @Test
    fun `continue watching loading succeeds with data`() = runTest {
        // Given
    ...
    @Test
    fun `continue watching loading fails with error`() = runTest {
        // Given
    ...
    @Test
    fun `legacy flows are properly initialized`() = runTest {
        // Given
        whenever(getContinueWatchingUseCase.refreshContinueWatching()).thenReturn(Result.success(emptyList()))
        whenever(checkTraktAuthorizationUseCase()).thenReturn(Result.success(false))
