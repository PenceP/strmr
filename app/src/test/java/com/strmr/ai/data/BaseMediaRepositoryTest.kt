...
import com.strmr.ai.data.Movie
import com.strmr.ai.data.TrendingMovie
...
class BaseMediaRepositoryTest {
    ...
    @Test
    fun `getTraktRatings returns cached ratings when not expired`() = runTest {
        // Given
        val traktId = 123
        val cachedRating = TraktRatingsEntity(
            traktId = traktId,
            rating = 8.5f,
            votes = 1000,
    ...
    @Test
    fun `getTraktRatings returns stale cache when expired`() = runTest {
        // Given
        val traktId = 123
        val expiredRating = TraktRatingsEntity(
            traktId = traktId,
            rating = 8.5f,
            votes = 1000,
    ...
    private class TestBaseMediaRepository(
        ...
        override suspend fun updateItemLogo(tmdbId: Int, logoUrl: String?) {}
