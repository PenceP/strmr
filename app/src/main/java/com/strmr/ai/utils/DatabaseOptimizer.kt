...
object DatabaseOptimizer {
     ...
     */
    val MIGRATION_PERFORMANCE_INDICES =
        object : Migration(12, 13) {
            override fun migrate(database: SupportSQLiteDatabase) {
                ...
                try {
                    ...
                    database.execSQL(
                        "CREATE INDEX IF NOT EXISTS idx_movies_trending_order ON movies(trendingOrder) WHERE trendingOrder IS NOT NULL",
                    )
                    database.execSQL(
                        "CREATE INDEX IF NOT EXISTS idx_movies_popular_order ON movies(popularOrder) WHERE popularOrder IS NOT NULL",
                    )
                    ...
                    database.execSQL(
                        "CREATE INDEX IF NOT EXISTS idx_tv_shows_trending_order ON tv_shows(trendingOrder) WHERE trendingOrder IS NOT NULL",
                    )
                    database.execSQL(
                        "CREATE INDEX IF NOT EXISTS idx_tv_shows_popular_order ON tv_shows(popularOrder) WHERE popularOrder IS NOT NULL",
                    )
                    ...
                    database.execSQL(
                        "CREATE INDEX IF NOT EXISTS idx_continue_watching_last_watched ON continue_watching(lastWatchedAt DESC)",
                    )
                    ...
                    database.execSQL(
                        "CREATE INDEX IF NOT EXISTS idx_episodes_show_season_episode ON episodes(showTmdbId, seasonNumber, episodeNumber)",
                    )
    ...
    class QueryProfiler {
        ...
        fun logQueryStats() {
            ...
            getQueryStats().forEach { (queryName, stats) ->
                Log.d(
                    TAG,
                    "  $queryName: avg=${stats.averageTime.toInt()}ms, " +
     ...
     */
    fun optimizeDatabaseBuilder(
        builder: androidx.room.RoomDatabase.Builder<StrmrDatabase>,
    ): androidx.room.RoomDatabase.Builder<StrmrDatabase> {
        return builder
    ...
    object CacheManager {
        ...
        fun isExpired(
            lastUpdated: Long,
            cacheHours: Int = DEFAULT_CACHE_DURATION_HOURS,
        ): Boolean {
