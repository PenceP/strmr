 ...
 */
@Singleton
class RemoteResourceLoader @Inject constructor(
     ...
     * 4. Return null
     */
    suspend fun resolveImageSource(url: String?): Any? = withContext(Dispatchers.IO) {
        when {
            url == null -> null
    ...
    private suspend fun downloadAndCache(url: String, targetFile: File): Boolean = withContext(Dispatchers.IO) {
        try {
