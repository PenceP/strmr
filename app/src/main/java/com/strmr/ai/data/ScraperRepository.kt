        ...
        private suspend fun processPremiumizeStreams(
            ...
            withContext(Dispatchers.IO) {
                try {
                    ...
                    val streamsWithoutCacheInfo =
                        processedStreams.filter { stream ->
                            val streamName = stream.name ?: ""
        ...
        private fun filterAndSortStreams(streams: List<Stream>): List<Stream> {
            ...
            return streams
                .filter { stream ->
                    ...
                    val isDolbyVision =
                        title.contains("dv") || title.contains("dolby") ||
                            name.contains("dolby-vision") || name.contains("dvhe")

                    val isProblematicHEVC =
                        title.contains("hevc") || title.contains("x265") ||
