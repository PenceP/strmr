        ...
        suspend fun searchMultiSource(query: String): SearchResults {
            return withContext(Dispatchers.IO) {
                ...
                try {
                    ...
                    Log.d(
                        "SearchRepository",
                        "🎬 Search results: ${combinedMovies.size} movies, ${combinedShows.size} shows, ${combinedPeople.size} people",
                    )
                    Log.d(
                        "SearchRepository",
                        "🔍 People search details: Trakt=${traktPeople.size}, TMDB=${tmdbPeople.size}, Combined=${combinedPeople.size}",
                    )
                ...
                if (existingItem == null) {
                    combinedResults.add(
                        SearchResultItem.Person(
                            ...
                            knownFor =
                                tmdbResult.known_for?.map { knownFor ->
