...
class GenericTraktRepository
    ...
    constructor(
         ...
         */
        fun getMoviesFromDataSource(config: DataSourceConfig): Flow<List<MovieEntity>> = dataSourceService.getMoviesFromDataSource(config)
         ...
         */
        suspend fun isMovieDataSourceEmpty(config: DataSourceConfig): Boolean = dataSourceService.isMovieDataSourceEmpty(config)

        /**
         * Check if TV data source is empty - delegates to DataSourceService
         */
        suspend fun isTvDataSourceEmpty(config: DataSourceConfig): Boolean = dataSourceService.isTvDataSourceEmpty(config)

        /**
         * Get movie data source count - delegates to DataSourceService
         */
        suspend fun getMovieDataSourceCount(config: DataSourceConfig): Int = dataSourceService.getMovieDataSourceCount(config)

        /**
         * Get TV data source count - delegates to DataSourceService
         */
        suspend fun getTvDataSourceCount(config: DataSourceConfig): Int = dataSourceService.getTvDataSourceCount(config)
         ...
         */
        suspend fun loadMovieDataSourcePage(
         ...
         * Orchestrates API calls, enrichment, and database operations
         */
        suspend fun loadTvDataSourcePage(
            config: DataSourceConfig,
            page: Int,
        ): Int {
            return try {
         ...
         */
        private suspend fun fetchMovieDataFromTrakt(
            config: DataSourceConfig,
            page: Int,
        ): List<Any> {
            return when {
         ...
         */
        private suspend fun fetchTvDataFromTrakt(
            config: DataSourceConfig,
            page: Int,
        ): List<Any> {
         ...
         */
        private suspend fun handleUserListEndpoint(
            config: DataSourceConfig,
            page: Int,
        ): List<Any> {
                        ...
                        dataSourceService.updateMovieDataSourceOrder(
                            ...
                            newOrder =
                                when (config.id) {
                                    ...
                                    "recommended_movies" -> entity.recommendedMoviesOrder
                                    else -> null
                                },
                        )
                    }
