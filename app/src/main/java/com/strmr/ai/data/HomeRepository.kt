...
class HomeRepository(
    ...
    suspend fun refreshContinueWatching(accountRepository: AccountRepository) {
        ...
        val entities =
            continueWatchingItems.mapIndexed { index, item ->
                val id =
                    when (item.type) {
    ...
    fun getUserProfile(username: String): Flow<TraktUserProfileEntity?> = traktUserProfileDao.getUserProfile(username)
    ...
    fun getUserStats(username: String): Flow<TraktUserStatsEntity?> = traktUserStatsDao.getUserStats(username)
