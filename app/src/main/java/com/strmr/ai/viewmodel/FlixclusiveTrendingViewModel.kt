...
class FlixclusiveTrendingViewModel @Inject constructor(
    ...
    fun paginateMovies(page: Int) {
        ...
        if (page <= lastLoadedPage && currentState.pagingState != PagingState.ERROR && page != 1) {
            Log.d("FlixclusiveTrendingViewModel", "🔄 Page $page already loaded (lastLoaded=$lastLoadedPage), skipping")
            return
        }
        ...
        paginationJob = viewModelScope.launch {
            try {
                ...
                Log.d("FlixclusiveTrendingViewModel", "✅ Pagination complete: ${updatedMovies.size} total movies, canPaginate=$canPaginate, nextPage=${_paginationState.value.currentPage}, lastLoaded=$lastLoadedPage")
